...
const fetchChatHistory = async (url: string): Promise<ChatHistory> => {
...
export function SidebarHistory({ user }: { user: User | undefined }) {
  ...
  const { data, fetchNextPage, hasNextPage, isFetching, isLoading } =
    useInfiniteQuery({
      queryKey: ["history"],
      queryFn: ({ pageParam }) =>
        fetchChatHistory(pageParam || `/api/history?limit=${PAGE_SIZE}`),
      ...
      initialPageParam: `/api/history?limit=${PAGE_SIZE}`,
      enabled: !!user,
    });
    ...
    toast.promise(deletePromise, {
      ...
      success: () => {
        ...
        queryClient.setQueryData(["history"], (oldData: any) => {
          ...
          return {
            ...oldData,
            pages: oldData.pages.map((page: ChatHistory) => ({
              ...page,
              chats: page.chats.filter((chat: any) => chat.id !== deleteId),
            })),
          };
        });
